...
export class PostSinglePage {
	...
	constructor(
		...
		if (this.navParams.get('thumbnail')) {
			this.post.better_featured_image.media_details.sizes.thumbnail.source_url = this.navParams.get('thumbnail');
		}
		if (this.navParams.get('title')) {
			this.post.title.rendered = this.navParams.get('title');
		}
		if (this.navParams.get('content')) {
			this.post.content.rendered = this.navParams.get('content');
		}
		...
		self.dataprovider.requestBlogAPI(
			'get',
			'wp-json/wp/v2/posts/' + self.post.id, 
			{},
			'',
